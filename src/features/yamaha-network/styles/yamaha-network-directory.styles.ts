...
export const NetworkDealerCard = styled.button<{ $isSelected: boolean; $isGrid: boolean }>`
  ...
  box-shadow: ${({ $isGrid, $isSelected }) =>
    $isGrid
      ...
      : "none"};
  contain-intrinsic-size: ${({ $isGrid }) => ($isGrid ? "240px" : "110px")};
  ...
  overflow: hidden;
  padding: 0;
  min-height: ${({ $isGrid }) => ($isGrid ? "240px" : "auto")};
  text-align: left;
  transition:
  ...
  &:hover {
    background: ${({ $isGrid, $isSelected }) =>
      $isGrid ? "#ffffff" : $isSelected ? "#fff5f4" : "#f8f9f6"};
...
export const NetworkDealerImage = styled.div<{ $isGrid: boolean }>`
  ...
  min-height: ${({ $isGrid }) => ($isGrid ? "120px" : "80px")};
  overflow: hidden;
  position: relative;
  width: ${({ $isGrid }) => ($isGrid ? "100%" : "100px")};
...
export const NetworkDealerBody = styled.div<{ $isGrid: boolean }>`
  ...
  flex: 1 1 auto;
  gap: 4px;
  padding: ${({ $isGrid }) => ($isGrid ? "14px 16px 16px" : "14px 16px")};
`;
...
export const NetworkDealerLocality = styled.span`
  color: #666c66;
  font-size: 12px;
